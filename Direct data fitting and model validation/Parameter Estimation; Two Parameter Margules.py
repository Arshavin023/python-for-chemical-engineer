#Fitting is used to fit experimental data into a proposed a model equation
# Objective is to minimize SSE = Sum square of error
# If you have x and y data then its called direct fitting
# If we have only x and Y(x) or dY/dx then its indirect fitting
# Model Equation = 𝐺𝐸𝑅𝑇=𝑥1∙𝑥2∙(𝐴21∙𝑥1+𝐴12∙𝑥2)
...
def F2M(PAR,x1,x2,GERTdat):
    ...
    for i in range(0,len(x1)):
        GERTcalc[i] = x1[i]*x2[i]*(PAR[0]*x1[i]*+PAR[1]*x2[i])
    res=GERTcalc-GERTdat #This is error
    SSEcalc = sum(res**2)
    return SSEcalc

PAR = np.array([1,1])
P2M = minimize(F2M,PAR,args=(x1,x2,GERTdat))
print(P2M)

#recalculation using optimized parameters (PAR)
A21 = P2M.x[0]
A12 = P2M.x[1]
GERTcalc = np.zeros(len(x1))
for i in range(0,len(x1)):
    GERTcalc[i]=x1[i]*x2[i]*(A21*x1[i]+A12*x2[i])
...
plt.xlabel('x1')
plt.legend(['Two parameters Margules ','Experiment'],loc='best', fontsize=10)
