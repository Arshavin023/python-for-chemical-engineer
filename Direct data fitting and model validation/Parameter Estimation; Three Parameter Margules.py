...
# Model Equation = 𝐺𝐸𝑅𝑇=𝐴12∙𝐴21∙(𝑥1∙𝑥2)(𝑥1∙𝐴12+𝑥2∙𝐴21)
...
def F2M(PAR,x1,x2,GERTdat):
    ...
    for i in range(0,len(x1)):
        GERTcalc[i] = PAR[0]*PAR[1]*(x1[i]*x2[i])/(x1[i]*PAR[0]+x2[i]*PAR[1])
    res=GERTcalc-GERTdat #This is error
    SSEcalc = sum(res**2)
    return SSEcalc

P2M = minimize(F2M,[1,1],args=(x1,x2,GERTdat))
print(P2M)
...
plt.xlabel('x1')
plt.legend(['Three parameters Margules ','Experiment'],loc='best', fontsize=10)
